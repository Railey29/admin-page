...
export default function Level1to3Dashboard({ admin, onLogout }: Props) {
  ...
  const searchedPending = filteredPending.filter(matchesSearch);
  ...
  const reportPageCount = Math.max(
    1,
    Math.ceil(searchedReports.length / PAGE_SIZE),
  );
              ...
              >
                ...
                >
                  ...
                  {userInfo?.supporting_document_url && (
                    <>
                      <SectionTitle label="Supporting Document" />
